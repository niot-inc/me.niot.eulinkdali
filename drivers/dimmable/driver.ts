...
module.exports = class DimmableDriver extends Homey.Driver {

  /**
   * onInit is called when the driver is initialized.
   */
  async onInit() {
    this.log('DimmableDriver has been initialized');
  }
  ...
  async onPairListDevices() {
    ...
    return devices.filter(
      (device) => device.activeConnector === 'dali' && device.internalId === 'daliDimmableLight',
    ).map((device) => ({
      name: device.name,
      data: {
